...
class CustomerClustering:
    ...
    def prepare_data(self, rfm_data, scale_features=True):
        ...
        Args:
            rfm_data (pd.DataFrame): RFM features DataFrame
            scale_features (bool): Whether to scale features
            
        Returns:
            array: Prepared data for clustering
        """
        try:
    ...
    def find_optimal_clusters(self, X, max_clusters=10):
        ...
        Returns:
            dict: Optimal cluster information
        """
        try:
    ...
    def analyze_clusters(self, rfm_data, cluster_labels):
        ...
        Args:
            rfm_data (pd.DataFrame): Original RFM data
    ...
    def save_clustering_results(self, clustered_data, output_dir="clustering_results"):
        ...
        try:
            ...
            cluster_stats = clustered_data.groupby('segment').agg({
                'Recency': ['mean', 'std', 'min', 'max'],
                'Frequency': ['mean', 'std', 'min', 'max'],
                'Monetary': ['mean', 'std', 'min', 'max']
            }).round(2)
...
# Example usage
def main():
    """Example usage of the CustomerClustering class"""
